...
auto logger = MagicSingleton<CLog>::GetInstance();
...
{
public:
	LogOpearator(int level, int linenum, const std::string& filename, const std::string functionname, bool isabort = false) 
		: _logstrm(level),
		_level(level), 
		_linenum(linenum),
		_filename(filename), 
...
#define SYS_ERROR_LOG \
	LogOpearator(spdlog::level::err, __LINE__, __FILE__, __FUNCTION__, true).stream()
